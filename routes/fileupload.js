        ...
        if(err){
            ...
            connection.query("INSERT INTO picture VALUES('"+Mfile.originalFilename+"','123456','"+Mfile.path.replace(/\\/g,"/").replace("public/","")+"','','')",function (error, results, fields) {
                if (error) throw error;
                    ...
                    process.exec('python ~/album/image-python-sdk-v2.0/sample.py "'+Mfile.originalFilename+'"  ~/album/public/'+Mfile.path.replace(/\\/g,"/").replace("public/","")+" >> log.txt",function (error, stdout, stderr) {
                        if (error !== null) {
